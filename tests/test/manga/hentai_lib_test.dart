...
void main() {
  ...
  setUpAll(() async {
    interpreter = Interpreter.fromFile(
      path: scriptPath('manga/hentai_lib/main.capyscript'),
    );
    browser = RealBrowserInterceptorController(headless: true);

    // hentai_lib reuses the mangalib session — passProtector reads mangalib.me cookies
  ...
  group('getImageHeaders', () {
    test('returns a Map with Authorization header', () async {
